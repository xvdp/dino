...
def apply_masked_attention(attn, order=None):
    """ mask attentions from smaller to larger
    optionally pass order=range(6) or order=[]
    loose interpretation of what attention heads matter
    https://github.com/facebookresearch/dino/issues/92
    """
    multiheads = np.asarray([[[0, 0, 0], [0, 160, 215]],
                            [[0, 0, 0], [220, 55, 60]],
                            [[0, 0, 0], [245, 180, 0]],
                            [[0, 0, 0], [10, 120, 190]],
                            [[0, 0, 0], [40, 150, 100]],
                            [[0, 0, 0], [135, 75, 145]]], dtype=np.uint8)
    if order is None:
        order = np.argsort(attn.reshape(6, -1).sum(axis=1))[::-1].tolist()
    order = list(order)
    assert len(order) == len(attn)

    mask = np.ones_like(attn[0])
    out = np.zeros((*attn[0].shape, 3), dtype=np.uint8)
    for i in order:
        out +=  multiheads[i][(attn[i]*mask).astype(int)]
        mask = np.clip(mask - attn[i], 0, 1) 

    return out

#np.asarray([ multiheads[i][D.th_attn[i].astype(int)] for i in range(len(multiheads))]).sum(axis=0)